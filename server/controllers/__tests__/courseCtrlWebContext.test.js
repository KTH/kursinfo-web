import { getServerSideFunctions } from '../../utils/serverSideRendering'
...
function buildReq(overrides) {
  const req = { headers: { accept: 'application/json' }, body: {}, params: {}, ...overrides, url: 'someUrl' }
  return req
}
...
describe('webContext test', () => {
  ...
  test('webContext', async () => {
    const { renderStaticPage, getCompressedData } = getServerSideFunctions()
    const req = buildReq({
    ...
    // expect(renderStaticPage).toHaveBeenCalledWith({
    //   applicationStore: {},
    //   location: req.url,
    //   basename: serverConfig.proxyPrefixPath,
    //   context,
    // })

    expect(testResponse.html.context).toMatchObject(context)
  })
})
