...
describe('Component <CourseStatisticsPage> show compilation data in for course analysis', () => {
  test('choose all parameters, chose/unchoose different seasons (termin) and check compilation table for chosen and check extracting of data for previous years', async () => {
    const analyses2021 = {
      combinedAnalysesPerSchool: {
        schools: {
          ABE: {
            numberOfCourses: 45,
            numberOfUniqAnalyses: 9,
          },
          EECS: {
            numberOfCourses: 43,
            numberOfUniqAnalyses: 57,
          },
          ITM: {
            numberOfCourses: 345,
            numberOfUniqAnalyses: 19,
          },
          CBH: {
            numberOfCourses: 356,
            numberOfUniqAnalyses: 35,
          },
          SCI: {
            numberOfCourses: 67,
            numberOfUniqAnalyses: 4,
          },
        },
        totalCourses: 856,
        totalUniqPublishedAnalyses: 124,
      },
    ...
    axios.get.mockResolvedValue({ data: analyses2021 })

    render(<CourseStatisticsPageWithContext context={context_sv} />)
    const analysis = screen.getByLabelText(/kursanalys/i)
    ...
    expect(columnheaderNames).toMatchInlineSnapshot(`
      [
        "School",
        "Antal kurser som slutar vald termin",
        "Antal publicerade kursanalyser för kursomgångar som slutar vald termin",
      ]
    `)
    const numberOfColumns = columnheaderNames.length
    ...
    expect(table.getByRole('row', { name: /ABE 45 9/ })).toBeInTheDocument()
    expect(table.getByRole('row', { name: /ITM 345 19/ })).toBeInTheDocument()
    expect(table.getByRole('row', { name: /CBH 356 35/ })).toBeInTheDocument()
    expect(table.getByRole('row', { name: /EECS 43 57/ })).toBeInTheDocument()
    expect(table.getByRole('row', { name: /SCI 67 4/ })).toBeInTheDocument()
    expect(table.getByRole('row', { name: /Total 856 124/ })).toBeInTheDocument()
