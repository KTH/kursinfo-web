...
import { INFORM_IF_IMPORTANT_INFO_IS_MISSING, PROGRAMME_URL, MAX_1_MONTH, MAX_2_MONTH, COURSE_IMG_URL } from '../util/constants'
...
class RouterStore {
  ...
  getCourseDefaultInformation (courseResult, language) {
    return {
      ...
      course_main_subject: courseResult.mainSubjects && courseResult.mainSubjects.length > 0 ? courseResult.mainSubjects.join(', ') : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
      ...
      course_department_link: this.isValidData(courseResult.course.department.name, language) !== INFORM_IF_IMPORTANT_INFO_IS_MISSING[language] ? '<a href="/' + courseResult.course.department.name.split('/')[0].toLowerCase() + '/" target="blank">' + courseResult.course.department.name + '</a>' : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
  ...
  getSyllabusData (courseResult, semester = 0, language) {
    return {
      course_goals: courseResult.publicSyllabusVersions && courseResult.publicSyllabusVersions.length > 0 ? this.isValidData(courseResult.publicSyllabusVersions[semester].courseSyllabus.goals, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
      course_content: courseResult.publicSyllabusVersions && courseResult.publicSyllabusVersions.length > 0 ? this.isValidData(courseResult.publicSyllabusVersions[semester].courseSyllabus.content, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
      course_disposition: courseResult.publicSyllabusVersions && courseResult.publicSyllabusVersions.length > 0 ? this.isValidData(courseResult.publicSyllabusVersions[semester].courseSyllabus.disposition, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
      course_eligibility: courseResult.publicSyllabusVersions && courseResult.publicSyllabusVersions.length > 0 ? this.isValidData(courseResult.publicSyllabusVersions[semester].courseSyllabus.eligibility, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
      course_requirments_for_final_grade: courseResult.publicSyllabusVersions && courseResult.publicSyllabusVersions.length > 0 ? this.isValidData(courseResult.publicSyllabusVersions[semester].courseSyllabus.reqsForFinalGrade, language, true) : '',
      course_literature: courseResult.publicSyllabusVersions && courseResult.publicSyllabusVersions.length > 0 ? this.isValidData(courseResult.publicSyllabusVersions[semester].courseSyllabus.literature, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
      course_literature_comment: courseResult.publicSyllabusVersions && courseResult.publicSyllabusVersions.length > 0 ? this.isValidData(courseResult.publicSyllabusVersions[semester].courseSyllabus.literatureComment, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
  ...
  getRound (roundObject, language = 0) {
    const courseRoundModel = {
      ...
      round_selection_criteria: language === 0 ? roundObject.round.round_selection_criteria.en : roundObject.round.round_selection_criteria.sv
      round_type: roundObject.round.applicationCodes.length > 0 ? this.isValidData(roundObject.round.applicationCodes[0].courseRoundType.name, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language], // TODO: Map array
      round_application_link: this.isValidData(roundObject.admissionLinkUrl, language),
      round_part_of_programme: roundObject.usage.length > 0 ? this.getRoundProgramme(roundObject.usage, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language],
      ...
      round_category: roundObject.round.applicationCodes.length > 0 ? this.isValidData(roundObject.round.applicationCodes[0].courseRoundType.category, language) : INFORM_IF_IMPORTANT_INFO_IS_MISSING[language]
  ...
  getRoundSeats (max, min, language) {
    if (max === INFORM_IF_IMPORTANT_INFO_IS_MISSING[language] && min === INFORM_IF_IMPORTANT_INFO_IS_MISSING[language]) {
      return INFORM_IF_IMPORTANT_INFO_IS_MISSING[language]
    }
    if (max !== INFORM_IF_IMPORTANT_INFO_IS_MISSING[language]) {
      if (min !== INFORM_IF_IMPORTANT_INFO_IS_MISSING[language]) {
        return min + ' - ' + max
      }
      else {
        return 'Max: ' + max
  ...
  getDateFormat (date, language) {
    if (date === INFORM_IF_IMPORTANT_INFO_IS_MISSING[language] || language === 1) {
      return date
    }
 ...
 /** ***************************************************************************************************************************************** */
  ...
  @action getCourseEmployeesPost (key, type = 'multi', lang = 'sv') {
    ...
    return axios.post(this.buildApiUrl(this.paths.redis.ugCache.uri, { key: key, type: type }), this._getOptions(JSON.stringify(this.keyList))).then(result => {
      ...
      const emptyString = INFORM_IF_IMPORTANT_INFO_IS_MISSING[this.courseData.language]
  ...
  @action getCourseEmployees (key, type = 'examinator', lang = 0) {
    return axios.get(this.buildApiUrl(this.paths.redis.ugCache.uri, { key: key, type: type })).then(result => {
      this.courseData.courseInfo.course_examiners = result.data && result.data.length > 0 ? this.createPersonHtml(result.data, 'examiner') : INFORM_IF_IMPORTANT_INFO_IS_MISSING[this.courseData.language]
    }).catch(err => {
      if (err.response) {
        throw new Error(err.message, err.response.data)
      }
